...
### ⚠️ Known inconsistency: two non-identical `mgba_socket.lua` copies

The repo contains **two** files named `mgba_socket.lua`:

- `src/mGBA-0.10.5-win64/scripts/mgba_socket.lua` — the one actually loaded by a running mGBA instance in current practice (confirmed with the project owner). Implements the full protocol described above: JSON responses (via the vendored `lunajson`), the complete `GameState` field set (badges, money, party, battle flags, trainer ID), and a frame-queued button-press system.
- `src/scripts/mgba_socket.lua` — a much simpler, older-looking variant: plain CSV response (`"%d,%d,%d,%d\n"` = hp, map, x, y only), immediate (non-queued) button presses, no JSON, no party/badge/money/battle data at all.

These are **not kept in sync** and do not implement the same protocol. If you're debugging a mismatch between what the C# side expects (`GameStateResponse` has ~9 fields) and what an emulator instance actually returns, check *which* script that instance has loaded before assuming a C#-side bug. This repo's refactor/documentation pass deliberately did not merge or delete either file — reconciling them is a behavioral decision for a dedicated follow-up task, not a cosmetic one.
...
1. **`RewardCalculatorService` likely has a reward-shaping bug**: `CalculateReward` updates `_previousMoney` every call, but **never updates `_previousBadges` or `_previousLevels`**. Since `GetBadgeReward`/`GetLevelUpReward` compute their reward as `current - previous`, and `previous` never advances, the same badge/level-up reward would likely keep re-firing on every subsequent step after the first badge/level gain, rather than firing once. Needs confirmation via a real training run and, if confirmed, a deliberate fix (out of scope for this doc/refactor pass).
2. **Two diverged `mgba_socket.lua` copies** (see §4) — decide whether `src/scripts/mgba_socket.lua` should be deleted, updated to match, or kept as a deliberately simpler reference/test script.
3. **Duplicate `IExperienceRepository` DI registration** (see §5) — clarify intended lifetime and remove the redundant registration.
4. **`ExperienceReplay`, `MemoryCache`, `SumTree` appear unused** by the current Redis-backed data path — confirm whether they're dead code to remove or a planned local-buffer optimization to finish wiring up.
5. **Hardcoded absolute Windows path** in both `mgba_socket.lua` copies (`package.path`/`package.cpath` pointing at `D:\Programming\PokemonRedRL\...`) — breaks portability to any other machine/drive letter; not changed here since it can't be verified without a real mGBA run.
6. **`DQNTrainer.SelectAction` allocates a `new Random()` per call** — statistically harmless but wasteful and non-reproducible; consider a single shared/seedable `Random` instance if run-to-run reproducibility ever matters.
7. **No automated verification of the emulator protocol contract** — a change to `GameStateResponse`'s shape or the Lua JSON response would only be caught at runtime. A lightweight contract test (serialize a sample JSON payload matching the Lua script's `state` table, deserialize with `GameStateSerializer`) would catch drift cheaply.

## 7. Target direction: computer-vision-augmented agent (planned, not yet implemented)
...
2. **A reviewed and likely upgraded RL approach.** The current single-network DQN + prioritized replay + Redis-shared weights setup is a reasonable baseline but was never benchmarked against alternatives (Double/Dueling DQN improvements, n-step returns, PPO, etc.), and has at least one likely-live reward-shaping bug (§6.1). Before scaling training time/compute, the algorithm itself needs a deliberate review, not just a bigger network.
